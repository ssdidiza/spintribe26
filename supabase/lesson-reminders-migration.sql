...
create table if not exists public.lesson_reminders (
  ...
  -- One message of each kind per session per channel; lets the ITN
  -- handler and cron re-run idempotently.
  unique (session_id, channel, kind)
);

-- Upgrade path for databases created with the original 24h/1h-only check.
alter table public.lesson_reminders drop constraint if exists lesson_reminders_kind_check;
alter table public.lesson_reminders
  add constraint lesson_reminders_kind_check
  check (kind in ('confirmation', 'daily_digest', 'reminder_24h', 'reminder_1h'));
