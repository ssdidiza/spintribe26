...
function statusColor(status: FeedbackStatus) {
  if (status === "shipped") return "#4ade80";
  if (status === "planned") return "#ffb1c1";
  if (status === "closed") return "#b8b8b8";
  return "#ff4b35";
}
    ...
    <section className="glass-card overflow-hidden">
      <div className="px-5 py-4 border-b border-white/[0.06] flex items-center justify-between gap-3">
        <div className="min-w-0">
          <p className="text-[10px] font-semibold tracking-[0.08em] uppercase text-[#b8b8b8]">Beta Feedback</p>
          <p className="text-sm font-bold text-[#ffffff]">{admin ? "Suggestions queue" : "Send beta feedback"}</p>
        </div>
      ...
      {!admin && (
        <div className="p-5 space-y-3 border-b border-white/[0.06]">
            ...
            {CATEGORY_OPTIONS.map((option) => {
              ...
              return (
                <button
                  ...
                  style={{
                    borderColor: selected ? "rgba(255,75,53,0.45)" : "rgba(255,255,255,0.1)",
                    color: selected ? "#ff4b35" : "#b8b8b8",
                    background: selected ? "rgba(255,75,53,0.12)" : "rgba(255,255,255,0.04)",
                  }}
                >
          ...
          <input
            ...
            className="w-full rounded-xl border border-white/10 bg-white/[0.04] px-3 py-2 text-sm text-white placeholder:text-white/30 outline-none focus:border-[#ff4b35]/50"
          />
          <textarea
            value={body}
            ...
            className="w-full rounded-xl border border-white/10 bg-white/[0.04] px-3 py-2 text-sm text-white placeholder:text-white/30 outline-none focus:border-[#ff4b35]/50 resize-none"
          />
          <button
            type="button"
            onClick={submitFeedback}
            disabled={acting === "new" || !title.trim() || !body.trim()}
            className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-[#ff4b35] px-4 py-2.5 text-xs font-black text-white disabled:opacity-50"
          >
      ...
      {error && (
        <div className="mx-5 mt-4 rounded-xl border border-red-500/25 bg-red-500/10 px-3 py-2 text-xs text-[#ffb4ab]">
          {error}
        </div>
      )}
      ...
      <div className="divide-y divide-white/[0.06]">
        {loading ? (
          <div className="p-8 text-center text-sm text-[#b8b8b8]">Loading feedback...</div>
        ) : sortedItems.length === 0 ? (
          <div className="p-8 text-center text-sm text-[#b8b8b8]">No feedback yet.</div>
        ) : (
          sortedItems.map((item) => {
            ...
            return (
              <article key={item.id} className="p-5 space-y-4">
                <div className="flex items-start gap-3">
                  <button
                    ...
                    className={cn(
                      ...
                      item.hasVoted ? "border-[#ff4b35]/50 bg-[#ff4b35]/15 text-[#ff4b35]" : "border-white/10 bg-white/[0.04] text-[#b8b8b8]"
                    )}
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="mb-1 flex flex-wrap items-center gap-2">
                      ...
                      <span className="inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider text-[#b8b8b8]">
                        {categoryIcon(item.category)}
                        {item.category}
                      </span>
                    </div>
                    <h3 className="text-sm font-black text-white">{item.title}</h3>
                    <p className="mt-1 text-xs leading-relaxed text-[#b8b8b8]">{item.body}</p>
                    <p className="mt-2 text-[10px] text-[#b8b8b8]/50">
                ...
                {latestAdminMessage && (
                  <div className="rounded-xl border border-[#ff4b35]/25 bg-[#ff4b35]/10 px-3 py-2">
                    <p className="mb-1 inline-flex items-center gap-1 text-[10px] font-black uppercase tracking-wider text-[#ff4b35]">
                      <CheckCircle2 size={11} />
                      Admin response
                    </p>
                    <p className="text-xs leading-relaxed text-white/85">{latestAdminMessage.body}</p>
                  </div>
                )}
                ...
                {item.messages.length > 0 && admin && (
                  <div className="space-y-2">
                    {item.messages.slice(-3).map((message) => (
                      <div key={message.id} className="border-l border-white/10 pl-3">
                        <p className="text-[10px] font-bold text-[#b8b8b8]">
                          {message.isAdmin ? "Admin" : message.authorName} - {format(new Date(message.createdAt), "MMM d, HH:mm")}
                        </p>
                        <p className="mt-0.5 text-xs text-white/75">{message.body}</p>
                      </div>
                    ))}
                  </div>
                )}
                ...
                {admin && (
                  <label className="block">
                    <span className="text-[9px] uppercase tracking-wider text-[#b8b8b8]">Status</span>
                    <select
                      value={item.status}
                ...
                {canReply && (
                  <div className="flex gap-2">
                    <input
                      ...
                      className="min-w-0 flex-1 rounded-xl border border-white/10 bg-white/[0.04] px-3 py-2 text-xs text-white placeholder:text-white/30 outline-none focus:border-[#ff4b35]/50"
                    />
                    <button
                      type="button"
                      onClick={() => sendMessage(item.id)}
                      disabled={sendingMessage || !messageDrafts[item.id]?.trim()}
                      className="inline-flex w-11 flex-shrink-0 items-center justify-center rounded-xl bg-[#ff4b35] text-white disabled:opacity-50"
